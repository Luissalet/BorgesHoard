...
export default function Fuentes() {
  ...
  return (
    <div>
      <PageHeader title="Fuentes" description="Además de carpetas, Borges puede indexar tus propias conversaciones de Faustus: cada chat pasa a ser un documento citable." />
      ...
      {sources && sources.length === 0 && (
        <Empty title="Sin fuentes todavía">Conecta tu Faustus para que el asistente pueda citar lo que decidisteis en conversaciones pasadas.</Empty>
      )}
            ...
            <section key={s.id} className="panel-white">
              <div className="flex flex-wrap items-start gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="help truncate">{s.config?.base_url}</div>
                  <div className="help mt-1 num">
                    {st.documents ?? 0} chats indexados{sync.error_count ? ` · ${sync.error_count} con error` : ""} · última sincronización {when(sync.last_run)}
                  </div>
                </div>
              ...
              {p && (
                <div className="mt-3 text-[12px]">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {p.phase === "extracting" && <span className="num">{p.files_done} / {p.files_total} chats · {p.current_file}</span>}
