"""Stdio MCP bridge for Borges's Hoard.
...
app itself (`python -m borges`, detached, on the port of BORGES_URL) and waits
for it, so a workspace that starts before the app still gets its tools;
BORGES_BRIDGE_AUTOSTART=0 turns that off.
"""
...
BASE_URL = os.environ.get("BORGES_URL", "http://127.0.0.1:5184").rstrip("/")
TOKEN_FILE = Path(
    os.environ.get("BORGES_TOKEN_FILE")
    or Path(os.environ.get("BORGES_DATA_DIR") or ROOT / "data") / "mcp-token"
)
NOT_RUNNING = "Open Borges's Hoard (python -m borges) so the assistant can reach your library."


def _healthy() -> bool:
    try:
        response = httpx.get(f"{BASE_URL}/api/health", timeout=4, trust_env=False)
        return response.status_code == 200 and response.json().get("service") == "borges-hoard"
    except Exception:
        return False
...
def ensure_running(timeout_s: float = 45.0) -> bool:
    ...
    if _healthy():
        return True
    if os.environ.get("BORGES_BRIDGE_AUTOSTART", "1") == "0":
        return False
    port = urlparse(BASE_URL).port or 5184
    env = {**os.environ, "BORGES_PORT": str(port), "PORT_STRICT": "1", "PYTHONUNBUFFERED": "1"}
    ...
    logs = Path(os.environ.get("BORGES_DATA_DIR") or ROOT / "data") / "logs"
    logs.mkdir(parents=True, exist_ok=True)
    with open(logs / "borges-app.log", "ab") as out:
        subprocess.Popen([sys.executable, "-m", "borges"], cwd=ROOT, env=env, stdin=subprocess.DEVNULL,
...
def _token() -> str:
    env = os.environ.get("BORGES_TOKEN")
    if env:
        return env.strip()
    return TOKEN_FILE.read_text(encoding="utf-8").strip()


class BorgesBridge(FastMCP):
    ...
    def __init__(self, catalog: list[dict], instructions: str):
        super().__init__(name="borges-hoard", instructions=instructions)
...
def main() -> None:
    ...
    BorgesBridge(catalog, instructions).run(transport="stdio")
